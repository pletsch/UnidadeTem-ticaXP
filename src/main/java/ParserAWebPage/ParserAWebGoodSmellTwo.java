 ...
 */
public class ParserAWebGoodSmellTwo {
    ...
    private static ArrayList<String> obtemAListaDeDeputados(Scanner in) {
        ...
        while (in.hasNext()) {
            String deputado = in.next();
            if(deputado.contains("href=\"http://ww1.al.rs.gov.br/")){
                constroeAListaDeNomes(deputado + in.next(),listaDeDeputados);
            }
        }
        return listaDeDeputados;
    }

    private static ArrayList<String> constroeAListaDeNomes(String deputado,ArrayList<String> listaDeDeputados){
        String primeiroNome = "";
        String sobrenome = "";
